...
export async function createSession(
  ...
  try {
    const initialSlackResponse = await (
      await fetch(
        `https://slack.com/api/oauth.v2.access?code=${code}&client_id=${process.env.SLACK_CLIENT_ID}&client_secret=${process.env.SLACK_CLIENT_SECRET}&redirect_uri=${url}`,
        {
          method: "POST",
          headers: {
    ...
    if (!initialSlackResponse.ok) {
      throw new Error(
        "Failed to get access token: " + initialSlackResponse.error
      );
    }
    ...
    const profileResponse = await fetch(
      "https://slack.com/api/openid.connect.userInfo",
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${initialSlackResponse.authed_user.access_token}`,
        },
      }
    );
